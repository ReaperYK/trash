...
check_env()
{
  ...
  if [ ! -e "/usr/bin/pacman" ];
  then
    warn "You need to run this installer on an ArchLinux based OS that pacman can use! \n"
  fi
...
update_keyring()
{
  ...
  printf1 'Initializing keyring'
	printf "\n\n"
	pacman -Sy archlinux-keyring archlinux32-keyring --noconfirm
	return $SUCCESS
}
...
formatter() {
  ...
  if [ "$target_filesystem" = "ext4" ]; then
   format_command='mkfs.ext4 -m 0 -F'
...
reinitialize_keyring()
{
  ...
  printf "\n\n"
  chroot $CHROOT pacman -Syy --overwrite='*' --noconfirm archlinux-keyring archlinux32-keyring
...
setup_extra_packages()
{
  ...
  || { err "Install failed. Will try again..."; sleep 3; rm /mnt/var/lib/pacman/db.lck; clear; setup_extra_packages; return $SUCCESS; }
...
ask_x11_setup() {
  title "Arch Linux Setup > Desktop Envrionment"
  warn "Enabling X11 may result in a slight increase in resident memory usage."
  printf "\n"
  wprintf "[?] Setup X11 + LXDE Desktop [y/n]: "
  read ASK_X11_LXDE_SETUP
...
setup_x11_lxde()
{
  title "Arch Linux Setup > LXDE Setup"
  wprintf "[+] Setting up LXDE"
  printf "\n\n"
  chroot $CHROOT pacman -S lxde lxdm spectacle xorg-server xorg xorg-xinit xorg-xlsfonts --needed --noconfirm || 
  { warn "Install failed"; echo;}
...
easter_backdoor()
{
  ...
  title 'Game Over'
  wprintf "[+] Arch Linux32 installation successfull!"
...
check_pacman_issues() {
  ...
  wprintf '[+] Checking for pacman keyring problems'
  printf "\n\n"
  chroot /mnt pacman -S archlinux-keyring archlinux32-keyring --noconfirm
  if [ "$?" = "1" ]; then
   err "Error! Apparently the keyring is corrupted. Damn, I need to fix that before I can exit."
   rm -rvf /mnt/etc/pacman.d/gnupg
   chroot /mnt pacman-key --init
   chroot /mnt pacman-key --populate archlinux
   chroot /mnt pacman-key --populate archlinux32
   chroot /mnt pacman -S archlinux-keyring archlinux32-keyging --noconfirm ||
   { err "Could not solve the problem, you will need to manually fix the keyring after quitting and rebooting"; }
  fi
}
# perform sync
sync_disk()
...
update_package()
{
  ...
  printf "\n\n"
  chroot /mnt pacman -Syu --noconfirm --disable-download-timeout --overwrite="*"
...
main()
{
  ...
  if [ ! -e "/tmp/arch-install/first_boot" ]; then
   show_notice
   sleep_clear 1
  fi
